...
import { addDays, bedtimeMinutes, buildWorkoutSessions, entriesInWindow, isDue, preferredSleepEntries, todayLocal } from "./health-model";
...
/** Clock trends use one source per night and follow the shortest path across midnight. */
export function sleepTimingSeries(state: HealthState, metric: "bedtime" | "wakeTime", asOf = todayLocal(), days = 30): SeriesPoint[] {
  const nights = new Map(preferredSleepEntries(state.sleepEntries).map(entry => [entry.date, entry]));
  let previous: number | null = null;
  return Array.from({ length: days }, (_, index) => {
    const date = addDays(asOf, index - days + 1);
    const raw = bedtimeMinutes(nights.get(date)?.[metric] ?? "");
    if (raw === null) return { date, value: null };
    const value = previous === null ? raw : raw + 1440 * Math.round((previous - raw) / 1440);
    previous = value;
    return { date, value };
  });
}
