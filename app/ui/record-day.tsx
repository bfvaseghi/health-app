...
import { gripLabels, modeLabels, recurrenceLabels } from "./thought-loops";
...
export function RecordDay({ state, editableState, initialDate, today, open, onClose }: {
  ...
  return (
    <ModalFrame title="Daily record" onClose={onClose}>
      ...
      {day.thoughts.length ? <section className="tl-section" aria-label="Recurring thoughts"><h3 className="tl-caps">Recurring thoughts</h3><ul className="record-list">{day.thoughts.map((event) => <li key={event.id}><span>{event.grip ? gripLabels[event.grip] : event.recurrence ? recurrenceLabels[event.recurrence] : "Came up"}{event.response ? <span className="record-note record-note-inline">{event.response}</span> : null}</span><b>{event.at.slice(11, 16)}{event.mode ? ` · ${modeLabels[event.mode]}` : event.move === "passed" ? " · Passed" : event.move === "hooked" ? " · Stuck" : event.move === "later" ? " · Later" : ""}</b></li>)}</ul></section> : null}
