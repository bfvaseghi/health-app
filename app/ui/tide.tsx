...
import { plotSeries } from "../chart-series";
...
export function Tide({
  ...
  const { points, segments, gaps } = plotSeries(data);
  ...
  const x = (position: number) => pad.l + position * w;
  const y = (value: number) => pad.t + h - ((Math.min(hi, Math.max(lo, value)) - lo) / (hi - lo || 1)) * h;
  const xy = points.map((point) => [x(point.position), y(point.value)] as const);

  const baseline = pad.t + h + 6;
  const curves = segments.map((segment) => {
    const coordinates = segment.map((point) => [x(point.position), y(point.value)] as const);
    ...
    return { path, area: `${path} L${coordinates.at(-1)![0].toFixed(1)},${baseline} L${coordinates[0][0].toFixed(1)},${baseline} Z`, single: coordinates.length === 1 ? coordinates[0] : null };
  });
    ...
    >
      {readout || !isNow ? (
        <p className="tide-readout" aria-live="polite">
          <span>{isNow ? `latest · ${dateLabel(current.date, dateFormat)}` : dateLabel(current.date, dateFormat)}</span>
        </p>
      ) : null}
      <svg viewBox={`0 0 ${width} ${height}`} className="tide-plot" aria-hidden="true" onClick={onClick}>
        ...
        {gaps.map(([from, to]) => <line key={`${from.date}:${to.date}`} className="tide-gap" x1={x(from.position)} y1={y(from.value)} x2={x(to.position)} y2={y(to.value)} />)}
        {curves.map((curve, index) => <g key={index}>
          <path className="tide-area" d={curve.area} fill={`url(#tide-${gradientId})`} />
          <path className="tide-line" d={curve.path} />
          {curve.single ? <circle className="tide-now" cx={curve.single[0]} cy={curve.single[1]} r={3} /> : null}
        </g>)}
